...
use crate::{I2o2Builder, SchedulerClosed, opcode};
...
fn test_scheduler_noop(#[case] builder: I2o2Builder) {
    ...
    let (scheduler, handle) = builder.try_spawn::<()>().unwrap();
...
#[test]
fn failpoint_scheduler_spawn_fail_try_spawn() {
    let scenario = FailScenario::setup();
    let err = crate::create_and_spawn::<()>().unwrap_err();
    assert_eq!(err.kind(), ErrorKind::Other);
    scenario.teardown();
}

#[test]
fn failpoint_submission_worker_run_fail_try_spawn() {
    super::try_init_logging();
...
#[test]
fn failpoint_completion_worker_run_fail_try_spawn() {
    super::try_init_logging();
